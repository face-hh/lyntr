...
export const lyntObj = (userId: string) => {
    return {
        ...
        likeCount: sql<number>`(SELECT COUNT(*) FROM ${likes} WHERE ${likes.lynt_id} = ${lynts.id})`.as('likeCount'), likedByFollowed: sql<boolean>`exists(
...
export async function uploadAvatar(inputBuffer: Buffer, fileName: string, minioClient: any) {
    ...
    for (const shit of shits) {
        ...
        await minioClient.putObject(process.env.S3_BUCKET_NAME!, shit.filename, shit.buffer, shit.buffer.length, {
