...
export const POST: RequestHandler = async ({ request, cookies }) => {
    ...
    try {
        ...
        const existingFollow = await db.select()
            ...
            .where(
                and(
                    eq(followers.user_id, targetUserId),
                    eq(followers.follower_id, authenticatedUserId)
                )
            )
            .execute();
    ...
    try {
        const [isFollowing, isFollowedBy] = await Promise.all([
            ...
            db.select()
                ...
                .where(
                    and(
                        eq(followers.user_id, authenticatedUserId),
                        eq(followers.follower_id, targetUserId)
                    )
                )
                .execute()
        ]);
